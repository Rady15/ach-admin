...
const EmployeeDashboard = () => {
    ...
    const filteredTasks = filter === 'all' ? myTasks : myTasks.filter(task => task.status?.toLowerCase() === filter);
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400 text-sm">{t('youHave')} {myTasks.filter(t => t.status?.toLowerCase() === 'inprogress').length} {t('tasksInProgressToday')}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <GlassPanel className="p-6">
                    ...
                    <p className="text-2xl font-bold text-info font-numbers">{myTasks.filter(t => t.status?.toLowerCase() === 'inprogress').length}</p>
                </GlassPanel>
                <GlassPanel className="p-6">
                    <h3 className="text-slate-400 text-sm mb-1">{t('completedTasks')}</h3>
                    <p className="text-2xl font-bold text-success font-numbers">{myTasks.filter(t => t.status?.toLowerCase() === 'completed').length}</p>
                </GlassPanel>
                <GlassPanel className="p-6">
                    <h3 className="text-slate-400 text-sm mb-1">{t('completionRate')}</h3>
                    <p className="text-2xl font-bold text-warning font-numbers">
                        {myTasks.length > 0
                            ? Math.round((myTasks.filter(t => t.status?.toLowerCase() === 'completed').length / myTasks.length) * 100)
                            : 0}%
                    </p>
                </GlassPanel>
            </div>
            ...
            <GlassPanel className="p-6">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => setFilter('inprogress')} className={`px-3 py-1 rounded-lg text-xs ${filter === 'inprogress' ? 'bg-info text-white' : 'bg-white/5 text-slate-400'}`}>{t('inProgress')}</button>
                        <button onClick={() => setFilter('waitingforpayment')} className={`px-3 py-1 rounded-lg text-xs ${filter === 'waitingforpayment' ? 'bg-purple-500 text-white' : 'bg-white/5 text-slate-400'}`}>{t('waitingForPayment')}</button>
                ...
                <DataTable
                    ...
                    actions={(row) => (
                        <div className="flex gap-2">
                            {row.status?.toLowerCase() !== 'completed' && row.status?.toLowerCase() !== 'paid' && (
                                <button
